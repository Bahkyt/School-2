...
def upload_file():
    ...
    xasdf = io.BytesIO(file_pdf.read())

    if file_pdf and file_pdf.filename.endswith(".pdf"):
        doc = fitz.open(stream=xasdf, filetype="pdf")
        ...
        for page_number in range(len(doc)):
            ...
            for img_index, img in enumerate(images):
                ...
                lan = easyocr.Reader(["ru"])

                img = Image.open(file_io)
                num = np.array(img)
                text = lan.readtext(num)
                if text[2][1] == "Карамат Б.":
                    if text[1][1] == "2 000 т":
                        return redirect("/update-pay")
                    else:
                        return "<script>alert('Вы отправили неправильную сумму'); window.location = '/payment';</script>"
                else:
                    return "<script>alert('Вы отправили деньги не тому человеку'); window.location = '/payment';</script>"
    else:
        return "<script>alert('Вышло ошибка или файл другого типа'); window.location = '/payment';</script>"
